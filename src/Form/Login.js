...
const Login = () => {
    ...
    const onSubmit = async (data) => {
        ...
        const callApi = await fetchData('login', 'post', data)

        if (callApi.success == true) {
            setApiError({ api_success: "Login successfull !!", api_error: "" })
            ...
            setApiError({ api_success: "", api_error: callApi?.response ? callApi?.response : 'Login Failed' })
        }
    };
